...
**AI-native music player for DeepSeek Harness (dsh)** — a standard dsh bundle that ships a Cordis host half (Node BFF + AI tools) and a browser half (React player panel) in one npm package.
...
- **Deep plugin integration**: registers 11 model-facing tools via the dsh tools service (`music_search / play / control / queue / volume / favorite / sleep_timer / alarm / now_playing / lyric`) — DeepSeek discovers and calls them by JSON schema, no glue code needed
