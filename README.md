...
- **推荐**：登录时网易云每日个性化推荐；未登录时热歌/飙升/新歌榜随机推荐
...
2. **发现**：「曲库」Tab —— 顶部为平台推荐（登录=每日个性化，未登录=随机榜单），下方管理你的多个列表
...
6. **花再同步**：音箱 USB 连接电脑后自动启用；暂停自动回时钟界面
...
| `GET /recommend` | 推荐：登录→每日个性化；未登录→随机公开榜 |
