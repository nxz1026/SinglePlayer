...
const ALLOWED_AUDIO_HOSTS = ["music.126.net", "qq.com"];
/** 判断 host 是否在允许白名单内。 */
...
const toplist$1 = (query) => createRequest("/api/toplist", {}, createOption(query));
...
async function toplist() {
	try {
		const r = await invoke(lib.toplist, { timestamp: Date.now() });
		return (Array.isArray(r.body?.list) ? r.body.list : []).map((item) => ({
			id: String(item?.id ?? ""),
...
function installBuiltinProviders() {
	...
	if (saved.length) {
		...
		for (const id of allProviderIds()) setEnabled(id, savedSet.has(id));
	}
	persistEnabled();
}
//#endregion
...
async function buildRecommendSections() {
	...
	try {
		const toplist$2 = await toplist();
		if (toplist$2.length) {
			const dateSeed = /* @__PURE__ */ new Date();
			const seed = dateSeed.getFullYear() * 1e4 + (dateSeed.getMonth() + 1) * 100 + dateSeed.getDate();
			const shuffled = [...toplist$2];
			let randState = seed;
			const rand = () => {
			...
			for (const chart of picked) {
				const tracks = await chartTracksById(chart.id, 30).catch(() => []);
...
/** 红心获取超时（毫秒），防止卡住 */
const LIKED_TRACKS_TIMEOUT = 3e3;
...
async function getNeteaseCookie() {
	const { loadAuth } = await Promise.resolve().then(() => auth_exports);
...
async function buildShuffleMix() {
	...
	if (await getNeteaseCookie()) try {
		const liked = await withTimeout(likedTracks(300), LIKED_TRACKS_TIMEOUT, []);
		for (const track of liked) pushUnique(track);
	} catch {}
	if (pool.length < SHUFFLE_SIZE) {
		const needed = SHUFFLE_SIZE - pool.length;
		const chartPromises = FALLBACK_CHARTS.map((chartId) => chartTracksById(chartId, Math.min(30, needed)).catch(() => []));
...
function makeHaloRoutes() {
	return [
		makeGet(`${API_PREFIX}/halo/status`, async () => ({ halo: getHaloSync().status() })),
		makePost(`${API_PREFIX}/halo/config`, async (body) => {
			const patch = body.config ?? {};
			return { config: getHaloSync().setConfig(patch) };
		}),
		makePost(`${API_PREFIX}/halo/lyric`, async (body) => {
			getHaloSync().onLyric(String(body.text ?? ""));
			return {};
		}),
		makePost(`${API_PREFIX}/halo/song`, async (body) => {
			getHaloSync().onSong(String(body.name ?? ""), String(body.artist ?? ""));
			return {};
		}),
		makePost(`${API_PREFIX}/halo/state`, async (body) => {
			getHaloSync().onPlayState(body.playing === true);
			return {};
		}),
		makePost(`${API_PREFIX}/halo/notify/dismiss`, async () => {
			getHaloSync().dismissNotify();
			return {};
		}),
		makePost(`${API_PREFIX}/halo/command`, async (body) => {
...
function makeRoutes(ctx) {
	return [
		makeGet(`${API_PREFIX}/health`, async () => ({
			plugin: "dsh-music-huazai",
			version: "0.1.0",
			milestone: "M6"
		})),
		...
		makePost(`${API_PREFIX}/alarm/add`, async (body) => ({ alarm: addAlarm(String(body.time ?? ""), String(body.keyword ?? ""), body.label == null ? void 0 : String(body.label)) })),
